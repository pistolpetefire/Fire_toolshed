...
const navItems = [
  ...
  { to: p('/plan'), label: 'Exam 1 plan', icon: CalendarCheck },
  { to: p('/units'), label: 'Chapters', icon: ListOrdered },
  { to: p('/syllabus'), label: 'Syllabus', icon: GraduationCap },
  { to: p('/flashcards'), label: 'Flashcards', icon: Layers },
  { to: p('/acronyms'), label: 'Acronyms', icon: BookA },
...
export function Layout() {
  ...
  return (
    <div className="flex min-h-screen">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[100] focus:rounded-xl focus:bg-orange-600 focus:px-4 focus:py-2 focus:text-sm focus:font-semibold focus:text-white focus:shadow-lg"
      >
        Skip to content
      </a>
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 lg:flex">
        <div className="flex items-center gap-2.5 border-b border-slate-200 px-5 py-5 dark:border-slate-800">
          ...
          <div>
            <div className="font-display text-sm font-bold leading-tight">Fire Protection Hub</div>
            <div className="text-[11px] text-slate-500">FPST 1213 · Exam 1 · Sep 14</div>
          </div>
        </div>
        {nav}
          ...
          <aside className="absolute left-0 top-0 flex h-full w-72 flex-col bg-white shadow-xl dark:bg-slate-900 animate-slide-up">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-4 dark:border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-sm font-bold">Fire Protection Hub</span>
              </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-slate-200/80 bg-white/80 px-4 py-3 backdrop-blur-md dark:border-slate-800 dark:bg-slate-950/80 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <Link to={p('/plan')} className="hidden rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-900 sm:inline-flex dark:bg-amber-950 dark:text-amber-200">
              Exam 1 · Mon Sep 14
            </Link>
            <button
              type="button"
        ...
        <footer className="border-t border-slate-200 px-4 py-4 text-center text-xs text-slate-500 dark:border-slate-800 sm:px-6">
          Study Buddy · OSU FPST 1213 Fall 2026 · Progress saved in this browser · Not for use during exams
        </footer>
      </div>
    </div>
  );
