...
export function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-orange-600 via-orange-700 to-slate-900 p-6 text-white shadow-glow sm:p-8">
        ...
        <div className="relative">
          ...
          <p className="mt-2 max-w-2xl text-sm text-orange-100 sm:text-base">{COURSE_GOAL}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <StatPill icon={<BookOpen className="h-4 w-4 text-orange-100" />} label="Overall" value={`${overall}%`} />
      ...
      <section className="card border-amber-200 p-5 dark:border-amber-900">
        ...
        <div className="mt-3 max-w-md">
          <ProgressBar label="Exam 1 chapter mastery" value={exam1Pct} color="bg-orange-500" size="sm" />
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <Link to={p('/plan')} className="btn-primary text-sm">
            <CalendarCheck className="h-4 w-4" /> Open 10-day plan
          </Link>
          <a href={CANVAS_URL} target="_blank" rel="noreferrer" className="btn-secondary text-sm">
            <ExternalLink className="h-4 w-4" /> Canvas
          </a>
          <Link to={p('/acronyms')} className="btn-secondary text-sm">
            Acronyms
          </Link>
      ...
      <section className="card border-orange-200 p-5 dark:border-orange-800">
        ...
        <Link to={p('/plan')} className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-orange-700 hover:underline">
      ...
      <section className="card p-5">
        ...
        <ol className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
          {STUDY_METHOD.map((step, i) => (
            ...
            >
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-800 dark:bg-orange-950 dark:text-orange-300">
                {i + 1}
              </span>
              {step}
            </li>
          ...
          {exam1Units.map((unit) => {
            ...
            return (
              <Link
                key={unit.id}
                to={p(`/units/${unit.id}`)}
                className="card group flex items-center gap-3 p-4 transition hover:border-orange-300 hover:shadow-md dark:hover:border-orange-700"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-orange-50 text-sm font-bold text-orange-800 dark:bg-orange-950 dark:text-orange-300">
                  {unit.number}
                </div>
      ...
      {nextUnit && (
        <section className="card p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-orange-700">Recommended next</p>
          <h2 className="mt-1 font-display text-lg font-semibold">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <Link to={p('/flashcards')} className="card p-5 transition hover:border-orange-300">
        ...
        <Link to={p('/quizzes')} className="card p-5 transition hover:border-orange-300">
        ...
        <Link to={p('/syllabus')} className="card p-5 transition hover:border-orange-300">
          ...
          <ul className="space-y-2">
            {recentQuizzes.map((q) => (
              <li key={q.id} className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 text-sm dark:bg-slate-800/50">
                ...
                <span className={`font-semibold tabular-nums ${q.percentage >= 70 ? 'text-orange-700' : 'text-amber-700'}`}>
                  {q.percentage}%
                </span>
              </li>
            ))}
