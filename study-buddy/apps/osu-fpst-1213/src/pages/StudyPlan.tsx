...
export function StudyPlan() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <p className="text-sm font-medium text-orange-700 dark:text-orange-400">Exam 1 countdown · start today</p>
        <h1 className="page-title">10-day study plan</h1>
      ...
      <section className="rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">
        <div className="flex gap-2">
          ...
          <p>
            Exam points come from <strong>lecture, Canvas, and the assigned FPHB chapters</strong> — not this app. Each task is tagged Class / Canvas / FPHB / This app. Read the slides and handbook pages before you drill.
          </p>
        </div>
      </section>
      ...
      <section className="card p-5">
        ...
        <div className="mt-3">
          <ProgressBar value={done} max={allTasks.length} color="bg-orange-500" />
        </div>
      </section>
      ...
      {tasks.map((task) => {
        ...
        return (
          ...
          >
            ...
            >
              <CheckCircle2
                className={`h-5 w-5 ${checked ? 'text-orange-600' : 'text-slate-300 dark:text-slate-600'}`}
              />
            </button>
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-400">
                ...
                {task.href && (
                  <Link
                    to={p(task.href)}
                    className="inline-flex items-center gap-1 font-semibold text-orange-700 hover:underline dark:text-orange-400"
                  >
