...
export function NotFound() {
  return (
    <div className="mx-auto flex min-h-[50vh] max-w-lg flex-col items-center justify-center px-4 text-center">
      <p className="font-display text-6xl font-bold text-orange-600">404</p>
      <h1 className="mt-3 font-display text-2xl font-bold text-slate-900 dark:text-white">Page not found</h1>
      <p className="mt-2 text-sm text-slate-500">That route doesn&apos;t exist in the FPST 1213 Fire Protection Hub.</p>
      <div className="mt-6 flex flex-wrap justify-center gap-2">
        <Link to={p('/')} className="btn-primary">
          <Home className="h-4 w-4" /> Dashboard
        </Link>
        <Link to={p('/plan')} className="btn-secondary">
          <CalendarCheck className="h-4 w-4" /> Exam 1 plan
        </Link>
