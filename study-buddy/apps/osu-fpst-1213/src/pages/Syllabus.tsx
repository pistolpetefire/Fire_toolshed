      ...
      <section className="card p-5 sm:p-6">
        ...
        <ul className="mt-3 space-y-2 text-sm text-slate-600 dark:text-slate-300">
          <li className="flex items-start gap-2">
            ...
            <a className="text-orange-700 hover:underline dark:text-orange-400" href={`mailto:${INSTRUCTOR.email}`}>
              {INSTRUCTOR.email}
            </a>
          </li>
          <li className="flex items-start gap-2">
        ...
        <ul className="mt-2 space-y-1 text-sm text-slate-600 dark:text-slate-300">
          {TAS.map((ta) => (
            <li key={ta.email}>
              {ta.name} ·{' '}
              <a className="text-orange-700 hover:underline dark:text-orange-400" href={`mailto:${ta.email}`}>
                {ta.email}
              </a>
              {ta.office ? ` · ${ta.office}` : ''}
            </li>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {EXAM_DATES.map((ex) => (
            <div key={ex.id} className="card p-4">
              ...
              {ex.id === 1 && (
                <Link to={p('/plan')} className="mt-3 inline-block text-sm font-semibold text-orange-700 hover:underline">
