...
export function UnitDetail() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to={p('/units')} className="mb-3 inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-orange-700">
          <ArrowLeft className="h-4 w-4" /> All chapters
        </Link>
        <p className="text-sm font-medium text-orange-700 dark:text-orange-400">
          {EXAM_BLOCKS.find((b) => b.id === unit.examBlock)?.title} · {unit.chapter}
          {!unit.ready ? ' · content after Exam 1' : ''}
        </p>
...
function UnitQuizLinks({ unitId, ready }: { unitId: UnitId; ready: boolean }) {
  ...
  return (
    <section className="card p-4">
      <h2 className="text-sm font-semibold">Study this chapter</h2>
      <div className="mt-3 flex flex-wrap gap-2">
        <Link to={p(`/units/${unit.id}?mode=quiz`)} className="btn-secondary text-xs">
          <BrainCircuit className="h-3.5 w-3.5" /> Chapter quiz
        </Link>
...
function UnitStepper({
  ...
  return (
    <div className="card p-3 sm:p-4">
      <div className="mb-3">
        <ProgressBar label="Chapter mastery" value={mastery} max={100} size="sm" color="bg-orange-500" />
      </div>
        ...
        {items.map(({ id, label, icon: Icon, done }) => (
          ...
          >
            <span className="relative">
              ...
              {done && mode !== id && <CheckCircle2 className="absolute -right-2 -top-2 h-3 w-3 text-orange-500" />}
            </span>
            {label}
          </button>
        ))}
    ...
    <div className="card p-5">
      ...
      <div className="mt-4 space-y-2">
        {q.options.map((opt, idx) => {
          let cls = 'border-slate-200 hover:border-orange-300 dark:border-slate-700';
          if (revealed) {
            if (idx === q.correctIndex) cls = 'border-orange-500 bg-orange-50 dark:bg-orange-950/40';
...
function PracticePanel({ unitId, onQuiz }: { unitId: UnitId; onQuiz: () => void }) {
  ...
  if (!q) {
    return <p className="text-sm text-slate-500">No practice items for this chapter yet — they land after Exam 1.</p>;
  }
...
function QuizPanel({ unitId, onReview }: { unitId: UnitId; onReview: () => void }) {
  ...
  if (questions.length === 0) {
    return <p className="text-sm text-slate-500">No quiz items for this chapter yet.</p>;
  }
  ...
  if (finished) {
    ...
    return (
      <div className="card mx-auto max-w-lg p-8 text-center">
        ...
        <p className="mt-2 text-4xl font-bold tabular-nums text-orange-700">{pct}%</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between text-sm text-slate-500">
        <span>Scored chapter quiz · no going back</span>
...
function ReviewPanel({ unitId }: { unitId: UnitId }) {
  ...
  if (!up.lastMistakes.length) {
    return (
      <div className="card p-6 text-sm text-slate-600 dark:text-slate-300">
        No missed items stored yet. Take the chapter quiz and misses will land here with explanations.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-slate-500">Misses from your last scored quiz on this chapter.</p>
      {up.lastMistakes.map((m) => (
        <div key={m.questionId + m.prompt.slice(0, 12)} className="card p-4 text-sm">
          ...
          <p className="flex items-center gap-1 text-orange-700 dark:text-orange-400">
