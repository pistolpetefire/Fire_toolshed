  ...
  if (id !== 1) {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <Link to={p('/quizzes')} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-orange-700">
    ...
    return (
      <div className="mx-auto max-w-lg space-y-6 text-center">
        ...
        <div className="card p-8">
          ...
          <p className="mt-2 text-4xl font-bold tabular-nums text-orange-700">{percentage}%</p>
              ...
              <ul className="mt-2 space-y-3">
                {mistakes.map((m) => (
                  <li key={m.questionId + m.prompt.slice(0, 8)} className="rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800/50">
                    ...
                    <p className="text-orange-700 dark:text-orange-400">Correct: {m.correctAnswer}</p>
        ...
        {revealed && (
          <div
            ...
            className={`mt-5 rounded-xl p-4 text-sm ${
              isWrong ? 'bg-rose-50 dark:bg-rose-950/40' : 'bg-orange-50 dark:bg-orange-950/40'
            }`}
          >
            <div className="flex items-center gap-2 font-semibold">
              {isWrong ? (
                ...
                <>
                  <CheckCircle2 className="h-4 w-4 text-orange-600" /> Correct!
                </>
              )}
            </div>
    ...
    <div className="mt-5 space-y-2" role="listbox" aria-label="Answer choices">
      {q.options.map((opt, idx) => {
        ...
        if (revealed) {
          ...
          cls = 'border-orange-500 bg-orange-50 dark:bg-orange-950/40';
        }
        return (
          <button
            key={opt}
          ...
          {revealed && (
            <span className="text-xs">
              {answers[pair.left] === pair.right ? (
                <CheckCircle2 className="inline h-4 w-4 text-orange-500" />
              ) : (
