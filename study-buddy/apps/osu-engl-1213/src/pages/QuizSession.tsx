...
function QuizRunner({ type, examBlock }: { type: QuizType; examBlock?: ExamBlockId }) {
  ...
  if (questions.length === 0) {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <Link to={p('/quizzes')} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-violet-700">
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div>
        <Link to={p('/quizzes')} className="mb-3 inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-violet-700">
          <ArrowLeft className="h-4 w-4" /> Quizzes
        </Link>
        ...
        {!examBlock && (
          <div className="mt-3 flex flex-wrap gap-2">
            <FilterChip label="All units" active={!unitFilter} onClick={() => setUnit('all')} />
            {readyUnits.map((u) => (
              <FilterChip
                key={u.id}
                label={u.chapter}
        ...
        {examBlock && (
          <p className="mt-2 text-sm text-slate-500">{examBlockLabel(examBlock)} · original items, not Canvas.</p>
        )}
