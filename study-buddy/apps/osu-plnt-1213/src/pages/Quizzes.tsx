        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {EXAM_BLOCKS.map((block) => {
            ...
            return (
              <div
                key={block.id}
                className={`card flex flex-col p-4 ${live ? 'hover:border-emerald-300 hover:shadow-md' : 'opacity-70'}`}
              >
                {live ? (
                  <Link to={p(`/quizzes/exam/${block.id}`)} className="group flex flex-1 flex-col">
                    ...
                    <span className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-emerald-700 group-hover:gap-1.5">
                ...
                {block.id === 1 && (
                  <Link
                    ...
                    className="mt-3 inline-flex items-center gap-1 border-t border-slate-100 pt-3 text-xs font-semibold text-slate-600 hover:text-emerald-700 dark:border-slate-800"
                  >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {QUIZ_TYPES.map(({ type, title, description, icon: Icon, color }) => (
          <Link
            key={type}
            to={p(`/quizzes/${type}`)}
            className="card group flex flex-col p-5 transition hover:border-emerald-300 hover:shadow-md"
          >
            ...
            <span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-emerald-700 group-hover:gap-1.5">
          ...
          {courseUnits
            ...
            .map((u) => (
              <Link
                ...
                className="rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-emerald-100 dark:bg-slate-800 dark:text-slate-200"
              >
                {u.chapter} quiz
              </Link>
            ))}
          ...
          <ul className="space-y-2">
            {history.slice(0, 8).map((h) => (
              ...
              >
                ...
                <span className={`font-semibold tabular-nums ${h.percentage >= 70 ? 'text-emerald-700' : 'text-amber-700'}`}>
                  {h.percentage}%
                </span>
              </li>
            ))}
