...
import { builtInFlashcards, unitFlashcardCount } from '../data/flashcards';
...
import { isUnitId, type Flashcard, type SRSRating, type UnitId } from '../types';
...
export function Flashcards() {
  ...
  const deck = useMemo(() => {
    let cards = allCards;
    if (unitFilter !== 'all') {
      cards = cards.filter((c) => c.unitId === unitFilter);
    } else {
      cards = cards.filter((c) => courseUnits.find((u) => u.id === c.unitId)?.ready || c.custom);
    }
    if (dueOnly) {
    ...
    return cards;
  }, [allCards, unitFilter, dueOnly, progress.cardProgress]);
  ...
  const addCustom = (front: string, back: string, unitId: UnitId) => {
    ...
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="page-subtitle">Organized by chapter. Rate Hard / Good / Easy for spaced repetition.</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {readyUnits.map((u) => (
          ...
          >
            {u.chapter} ({unitFlashcardCount(u.id)})
          </button>
        ))}
        <button
          type="button"
        ...
        {sessionReviewed > 0 && (
          <span className="badge bg-emerald-50 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
            Session: {sessionReviewed}
          </span>
        )}
      </div>
        ...
        <>
          ...
          <div className="flip-card mx-auto h-64 w-full max-w-xl sm:h-72">
            ...
            >
              ...
              <div className="flip-card-face flip-card-back card flex flex-col items-center justify-center bg-emerald-50 p-6 text-center shadow-md dark:bg-emerald-950/40">
                <span className="section-label mb-3">Answer</span>
                <p className="text-base leading-relaxed text-slate-800 dark:text-slate-100 sm:text-lg">{card?.back}</p>
          ...
          <div className="mx-auto flex max-w-xl flex-col gap-3">
            {!flipped ? (
              ...
              <div className="grid grid-cols-3 gap-2" role="group" aria-label="Rate card difficulty">
                <button
                  type="button"
                  className="btn bg-rose-100 text-rose-800 hover:bg-rose-200 dark:bg-rose-950 dark:text-rose-200"
                  onClick={() => rate('hard')}
                >
                ...
                <button
                  type="button"
                  className="btn bg-amber-100 text-amber-900 hover:bg-amber-200 dark:bg-amber-950 dark:text-amber-200"
                  onClick={() => rate('good')}
                >
                ...
                <button
                  type="button"
                  className="btn bg-emerald-100 text-emerald-900 hover:bg-emerald-200 dark:bg-emerald-950 dark:text-emerald-200"
                  onClick={() => rate('easy')}
                >
      ...
      {showAdd && (
        <AddCardModal
          defaultUnit={unitFilter === 'all' ? 'unit-1' : unitFilter}
...
function AddCardModal({
  ...
  defaultUnit: UnitId;
  onClose: () => void;
  onSave: (front: string, back: string, unitId: UnitId) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="card relative z-10 w-full max-w-md animate-slide-up p-6 shadow-xl">
        <h3 className="font-display text-lg font-bold">Add custom flashcard</h3>
        <p className="mt-1 text-sm text-slate-500">Saved in this browser only.</p>
        <div className="mt-4 space-y-3">
          <div>
            <label className="mb-1 block text-xs font-semibold">Front (question)</label>
            <textarea className="input min-h-[80px] resize-y" value={front} onChange={(e) => setFront(e.target.value)} autoFocus />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold">Back (answer)</label>
            <textarea className="input min-h-[80px] resize-y" value={back} onChange={(e) => setBack(e.target.value)} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold">Chapter</label>
            <select className="input" value={unitId} onChange={(e) => setUnitId(e.target.value as UnitId)}>
              {ready.map((u) => (
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            onClick={() => onSave(front.trim(), back.trim(), unitId)}
          >
            Save card
          </button>
        </div>
