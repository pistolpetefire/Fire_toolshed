...
export function Settings() {
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `study-buddy-osu-plnt-1213-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
          ...
          {(['system', 'light', 'dark'] as const).map((t) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-sm font-medium capitalize ${
                progress.theme === t ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800'
              }`}
      ...
      <section className="card space-y-3 p-5">
        ...
        {importMsg && (
          <p className={`flex items-center gap-1 text-sm ${importMsg.type === 'ok' ? 'text-emerald-700' : 'text-rose-600'}`}>
      ...
      <section className="card p-5 text-sm text-slate-500">
        ...
        <p>
          PLNT 1213 Agronomy Hub v1.0.0 — personal study tool. Not an official OSU exam. AI may not be used on Haggard’s
