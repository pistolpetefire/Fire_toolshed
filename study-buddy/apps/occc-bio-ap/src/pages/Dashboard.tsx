...
import { COURSE_GOAL, courseUnits, FINAL_REVIEW } from '../data/courseUnits';
...
export function Dashboard() {
  ...
  const unitsDone = courseUnits.filter((u) => isUnitComplete(progress.units[u.id])).length;
    ...
    <div className="space-y-8">
      ...
      <section className="card p-5">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="font-display text-lg font-semibold">Study method (Senter objectives)</h2>
        </div>
      ...
      <section className="card border-brand-200 p-5 dark:border-brand-800">
        <p className="text-xs font-semibold uppercase tracking-wide text-brand-600">Exam 2 focus</p>
        <h2 className="mt-1 font-display text-lg font-semibold">Official Unit Two study guide (Ch 3 &amp; 4)</h2>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          25-item Unit Two guide, Unit 2 LOs (fluids, meiosis, named conditions), cell lecture, and histology /
          epithelial lab. Then take the two-unit practice exam.
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          <Link to={p('/quizzes/exam/2/guide')} className="btn-primary text-sm">
            Open Exam 2 study guide
          </Link>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <section className="lg:col-span-2">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <QuickCard
              to={p(nextUnit ? `/units/${nextUnit.id}` : '/units')}
              icon={<ListOrdered className="h-5 w-5" />}
              title={nextUnit ? `Continue Unit ${nextUnit.number}` : 'Unit path'}
              desc={
                nextUnit
                  ? `${nextUnit.title} · ${unitsDone}/10 units mastered`
                  : 'Official 10-unit BIO 1314 path'
              }
              color="bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300"
              badge={nextUnit ? 'Next' : undefined}
            />
            <QuickCard
      ...
      <section className="card p-5">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-display text-lg font-semibold">Official BIO 1314 unit path</h2>
              <p className="text-sm text-slate-500">
                Senter Fall 2026 · 10 units · 5 exam blocks · {unitsDone}/10 mastered
              </p>
            </div>
          </div>
          <Link to={p('/syllabus')} className="inline-flex items-center gap-1 text-sm font-medium text-brand-600 hover:underline dark:text-brand-400">
            <GraduationCap className="h-4 w-4" /> Full syllabus
          </Link>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="tabular-nums text-xs text-slate-400">{mastery}%</span>
                </div>
                <p className="mt-1 line-clamp-1 text-xs text-slate-500">
                  Exam {unit.examBlock} · {unit.chapters.join(' · ')} · {unit.topics.slice(0, 3).join(' · ')}
                </p>
              </Link>
            );
          })}
        ...
        <section className="card p-5">
          ...
          {weak.length === 0 ? (
            ...
            <ul className="space-y-2">
              {weak.map((w) => (
                <li key={w.unitId ?? w.systemId ?? w.name}>
                  <Link
                    to={p(w.href)}
                ...
                >
                  <div>
                    <p className="font-medium capitalize text-slate-800 dark:text-slate-100">
                      {q.unitId
                        ? `${q.unitId.replace('unit-', 'Unit ')} quiz`
