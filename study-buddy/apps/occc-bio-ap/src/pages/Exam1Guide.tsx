  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex flex-wrap gap-2">
          <Link to={p('/quizzes/exam/1')} className="btn-primary text-xs">
            <ListChecks className="h-3.5 w-3.5" /> Exam 1 practice quiz
          </Link>
      ...
      <div className="flex flex-wrap justify-between gap-2 border-t border-slate-200 pt-4 dark:border-slate-800">
        <Link to={p('/units/unit-1')} className="btn-ghost text-sm">
          <BookOpen className="h-4 w-4" /> Unit 1 lesson
        </Link>
        <Link to={p('/quizzes/exam/1')} className="btn-primary text-sm">
          Start Exam 1 practice <ArrowRight className="h-4 w-4" />
        </Link>
      </div>
    </div>
  );
      ...
      {open && (
        <div className="border-t border-slate-100 px-4 py-3 text-sm text-slate-700 dark:border-slate-800 dark:text-slate-200">
          ...
          <p className="mt-2 text-xs text-slate-400">
            Confirm in Saladin 10e · {item.unitId === 'unit-2' ? 'Ch 2' : 'Ch 1 / Appendix A'} · then check the circle
            when you can say it without looking.
          </p>
        </div>
      )}
    </li>
