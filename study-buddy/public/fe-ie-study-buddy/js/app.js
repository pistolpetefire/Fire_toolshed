/**
 * FE Industrial Practice — diagnostic then targeted reps.
 */
(function () {
  'use strict';
  const STORAGE_KEY = 'study-buddy:fe-ie-study-buddy:progress-v1';
  const VERSION = '1.1.0';
  const HB_DB = 'fe-ie-handbook-v1';
  ...
  function questions() { return window.IE_QUESTIONS || []; }
  function topics() { return window.IE_TOPICS || []; }
  function mathProblems() { return window.IE_MATH || []; }
  function formulas() { return window.IE_FORMULAS || []; }
  ...
  function showView(name) {
    for (const key of ['home', 'topics', 'quiz', 'math', 'missed', 'formulas', 'manual', 'settings', 'resources']) {
  ...
  function band(pct) {
    const w = (window.IE_STRENGTH && window.IE_STRENGTH.weak) || 50;
    const s = (window.IE_STRENGTH && window.IE_STRENGTH.strong) || 80;
  ...
  function renderTopics() {
    ...
    el.topicGrid.innerHTML = scored
      .map((t) => {
        ...
        return `<div class="topic-wrap">
          <button type="button" class="topic-card band-${b}" data-topic="${t.id}">
            <strong>${escapeHtml(t.label)}</strong>
            <span>${escapeHtml(t.blurb)}</span>
            <span class="stat"><span class="strength-badge ${b}">${b.replace('-', ' ')}</span>${n} drill · ${label}</span>
          </button>
          <button type="button" class="btn btn-ghost btn-primer" data-res="${t.id}">Read first + sources</button>
        </div>`;
      })
      .join('');
    ...
    el.topicGrid.querySelectorAll('[data-res]').forEach((btn) => {
      btn.addEventListener('click', () => openResources(btn.getAttribute('data-res')));
    });
  }
  ...
  function onAnswer(q, letter, btnEl) {
    ...
    if (!showTutor) {
      ...
      const pack = window.IE_TUTORING?.buildMcqTutor(q, letter, correct);
  ...
  function renderTutor(pack, correct) {
    ...
    return `
      ...
      ${renderResourceBlock(pack.resources, '6 · Free reading (not the exam handbook)')}`;
  }

  function renderResourceBlock(res, heading) {
    const links = (res && res.links) || [];
    if (!links.length) return '';
    const lis = links
      .slice(0, 3)
      .map(
        (l) => `<li><a href="${escapeHtml(l.url)}" target="_blank" rel="noopener">${escapeHtml(l.title)}</a>
          <span class="resource-meta">${escapeHtml(l.org)} · ${escapeHtml(l.license)}</span>
          <div class="resource-why">${escapeHtml(l.why || '')}</div></li>`
      )
      .join('');
    return `<div class="tutor-block resource-block"><strong>${escapeHtml(heading)}</strong>
      <ul class="resource-list">${lis}</ul></div>`;
  }

  function renderPrimer(topicId) {
    const pack = typeof window.IE_RESOURCES_FOR === 'function' ? window.IE_RESOURCES_FOR(topicId) : { primer: [], links: [] };
    if (!(pack.primer || []).length) return '';
    return `<div class="primer-card">
      <strong>Read this first</strong>
      <ol>${pack.primer.map((p) => `<li>${escapeHtml(p)}</li>`).join('')}</ol>
      ${renderResourceBlock(pack, 'Open sources for this topic')}
    </div>`;
  }
  ...
  function renderMath() {
    ...
    el.mathList.innerHTML = list.map((p, i) => {
      const pack = window.IE_TUTORING?.buildWorkshopTutor(p) || {};
      return `<article class="card math-card">
        ...
        <div class="solution hidden" data-box="coach"><p>${escapeHtml(pack.writeOnExam || '')}</p>
          <p class="tutor-tip">${escapeHtml(pack.howToCheck || '')}</p>
          ${renderResourceBlock(pack.resources, 'Free reading')}</div>
      </article>`;
    }).join('');
  ...
  function handbookSectionsHtml() {
    const secs = (window.IE_HANDBOOK && window.IE_HANDBOOK.sections) || [];
    return secs
      .map(
  ...
  function openHandbookOverlay() {
    ...
    if (!ov || !body) {
      window.open('./handbook.html', 'fe-ie-handbook', 'width=720,height=900');
      return;
    }
  ...
  function openHandbookWindow() {
    window.open('./handbook.html', 'fe-ie-handbook', 'width=720,height=900,scrollbars=yes');
  }
  ...
  function renderResources(focusId) {
    const box = document.getElementById('resource-list');
    if (!box) return;
    const note = (window.IE_RESOURCES && window.IE_RESOURCES.note) || '';
    box.innerHTML = `<p class="lead">${escapeHtml(note)}</p>` + topics().map((t) => {
      return `<article class="card resource-card" id="res-${t.id}">
        <h3>${escapeHtml(t.label)}</h3>
        <p class="sub">${escapeHtml(t.blurb)}</p>
        ${renderPrimer(t.id)}
        <div class="toolbar">
          <button type="button" class="btn btn-primary" data-topic="${t.id}">Drill this topic</button>
        </div>
      </article>`;
    }).join('');
    ...
    if (focusId) {
      document.getElementById('res-' + focusId)?.scrollIntoView({ block: 'start' });
    }
  }

  function openResources(topicId) {
  ...
  document.getElementById('btn-resources')?.addEventListener('click', () => openResources());
  document.getElementById('btn-manual')?.addEventListener('click', () => showView('manual'));
  document.getElementById('btn-settings')?.addEventListener('click', () => showView('settings'));
  document.getElementById('btn-reset')?.addEventListener('click', () => {
    if (!confirm('Reset all FE Industrial progress on this device?')) return;
